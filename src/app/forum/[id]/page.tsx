...
export default function ForumPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      try {
        // 게시글 가져오기
        const postResponse = await fetch(`/api/free-posts/${id}`)
        ...
        // 댓글 가져오기
        const commentsResponse = await fetch(`/api/free-posts/${id}/comments`)
        ...
        // 좋아요 상태 확인
        if (currentUser) {
          const likeResponse = await fetch(`/api/free-posts/${id}/like`)
          if (likeResponse.ok) {
  ...
  const handleLike = async () => {
    ...
    try {
      const response = await fetch(`/api/free-posts/${id}/like`, {
        method: 'POST'
      })
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      const response = await fetch(`/api/free-posts/${id}/comments`, {
        method: 'POST',
